...
- the OAuth/PKCE login flow as implemented client-side,
...
The client stores a bearer token (`usk_…`) in `~/.config/unison/config.json` with
`0600` permissions, or reads it from the `UNISON_TOKEN` environment variable. The
token is never logged or transmitted anywhere except the configured API host.
